...
class COCO(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, 
                splits=["train", "val", "test"], use_restval=False, cut_validation=False):
        roots = {}
        '''
        roots['train'] = {
        ...
        for split in splits:
            roots[split] = {
                'img': os.path.join(img_root, split),
                'cap': os.path.join(ann_root, f'uitviic_captions_{split}2017.json')
            }
        '''
        roots['trainrestval'] = {
            'img': (roots['train']['img'], roots['val']['img']),
        ...
        if id_root is not None:
            ids = {}
            ids['train'] = np.load(os.path.join(id_root, 'uitviic_train_ids.npy'))
            ids['val'] = np.load(os.path.join(id_root, 'uitviic_val_ids.npy'))
            if cut_validation:
                ids['val'] = ids['val'][:5000]
            ids['test'] = np.load(os.path.join(id_root, 'uitviic_test_ids.npy'))
            '''
            ids['trainrestval'] = (
                ids['train'],
